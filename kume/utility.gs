...
function convertToHtmlTableSingle(cellValue) {
  if (!cellValue) return '';

  const regex = /\[(.*?)\]\s+([^[]+)/g;
  const measurementRegex = /(\w+)\s+([\d.]+)/g;
  ...
  while ((match = regex.exec(cellValue)) !== null) {
    ...
    const measurementText = match[2];
    const measurements = {};

    let measureMatch;
    while ((measureMatch = measurementRegex.exec(measurementText)) !== null) {
      const key = measureMatch[1];
      const value = measureMatch[2];
      measurements[key] = value;
      headersSet.add(key);
    }

    sizes.push({ sizeLabel, measurements });
  }
  ...
  sizes.forEach(({ sizeLabel, measurements }) => {
